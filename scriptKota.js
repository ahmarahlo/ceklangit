...
// Menyimpan kota yang dicari ke riwayat pencarian
const saveToHistory = (city) => {
  if (history.includes(city)) return; // Jangan tambahkan kota yang sudah ada
  history = history.filter((item) => item !== city); // Menghapus kota yang sama jika sudah ada
...
const renderHistory = () => {
  ...
  history.forEach((city) => {
    fetch(`https://api.weatherapi.com/v1/current.json?key=${apiKey}&q=${city}`)
      ...
      .then((data) => {
        if (data.error) {
          console.error("Error fetching data for city:", city);
          return;
        }
        ...
        // Mengubah event listener untuk tombol "Lihat Detail"
        div.querySelector(".detail-button").addEventListener("click", () => {
          if (window.location.pathname.includes("kota.html")) {
            // Jika sudah di kota.html, pindah ke index.html dengan query string
            window.location.href = `index.html?city=${encodeURIComponent(
              city
            )}`;
          } else {
            // Jika belum di kota.html, tetap di halaman yang sama
...
// Menampilkan cuaca berdasarkan kota yang dipilih
const loadCityWeather = async (city) => {
  ...
  try {
    ...
    // Menampilkan nama kota, suhu, dan deskripsi cuaca
    ...
    // Menampilkan ramalan cuaca untuk 3 hari ke depan
...
// Fungsi untuk mencari kota berdasarkan input
...
// Menampilkan cuaca berdasarkan query string kota saat halaman dimuat
window.addEventListener("DOMContentLoaded", () => {
  renderHistory(); // Tampilkan riwayat dari localStorage
